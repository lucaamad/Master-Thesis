...
class ValidationDataset(IterableDataset):
    ...
    def __init__(self, num_samples, sampling_freq, intervals, dt, dim_X, device):
        ...
        self.theta = torch.zeros((num_samples, self.n_params), device=device)
        for i, (low, high) in enumerate(intervals):
            self.theta[:, i].uniform_(low, high)

        self.W_dt = torch.normal(0.0, float(np.sqrt(dt)), size=(num_samples, sampling_freq), device=device)

        # Generate all payoffs and X
        payoff_aritm = simulate_arithmetic_asian_option_payoff(num_samples, sampling_freq, dt, self.W_dt, self.theta, device)
        payoff_geom = simulate_geometric_asian_option_payoff(num_samples, sampling_freq, dt, self.W_dt, self.theta, device)
        self.X = simulate_X(num_samples, self.W_dt, dim_X, device)
        self.label = payoff_aritm - payoff_geom

        # Scale theta and X
        self.theta_scaled = scale_theta(self.theta, intervals)
        self.X_scaled = scale_X(self.X, dt, sampling_freq, dim_X)
