...
def setup_global_pca(N_calibration, sampling_freq, intervals, dt, device):
    ...
    # Generate theta, W and the label
    ...
    transformer.fit(theta.detach(), W_dt.detach(), label.detach(), grads, intervals)
...
class PEMCDataset(IterableDataset):
    ...
    def __iter__(self):
        for batch_idx in range(self.batches_per_epoch):
            ...
            transformed_features, scaled_label = self.transformer.transform(theta, W_dt, label)
...
class ValidationDataset(IterableDataset):
    ...
    def __iter__(self):
        yield self.transformed_features.detach(), self.label.detach()
